...
const BULL_STRATEGIES = ['?¨æ—¥å¼·å‹¢??,'è¿‘ä??¥å¼·?¢è‚¡','è¿‘å??¥å¼·?¢è‚¡','?‹å???,'çªç ´?‡ç?','çªç ´å£“å?','?›è?å¤?,'çªç ´è¶¨å‹¢ç·?];
const BEAR_STRATEGIES = ['?¨æ—¥å¼±å‹¢??,'è¿‘ä??¥å¼±?¢è‚¡','è¿‘å??¥å¼±?¢è‚¡','è·Œç ´å¸ƒæ?','è·Œç ´?‡ç?','ç©ºé ­?’å?','?›è?ç©?,'ç¶ æŸ±?¾å¤§'];
...
export function BullBearPanel({ symbol }: { symbol: string }) {
  ...
  const DIMENSIONS = [
    { key: 'trend',     label: 'è¶¨å‹¢' },
    { key: 'momentum',  label: '?•èƒ½' },
    { key: 'volume',    label: '?èƒ½' },
    { key: 'chips',     label: 'ç±Œç¢¼' },
    { key: 'pattern',   label: '?‹æ?' },
    { key: 'sentiment', label: '?…ç?' },
  ];
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          ...
          <>
            ...
            <div style={{ marginTop: 8, display: 'flex', justifyContent: 'center', gap: 8, flexWrap: 'wrap' }}>
              ...
              <span style={{ fontSize: 11, color: '#8B8FA8', alignSelf: 'center' }}>
                ç¶œå?è©•å? {score.overall}/100
              </span>
            </div>
          </>
        ) : (
          <div style={{ color: '#8B8FA8', fontSize: 13, padding: 24 }}>?«ç„¡è©•å?è³‡æ?</div>
        )}
      </div>
      ...
      <div style={{ background: '#0F1117', border: `1px solid ${BORDER}`, borderRadius: 8, padding: 12 }}>
        <div style={{ fontSize: 12, fontWeight: 700, color: '#fff', marginBottom: 8 }}>è¶¨å‹¢?›é?</div>
        <TrendStrengthChart signals={[]} isLoading={false} />
        <div style={{ fontSize: 11, color: '#8B8FA8', textAlign: 'center', marginTop: 6 }}>
          ?¤ä¸­è¨Šè?å°‡æ–¼?‹ç›¤å¾Œé¡¯ç¤?        </div>
      </div>

      {/* S5: 60-day bull/bear chart */}
      <div style={{ background: '#0F1117', border: `1px solid ${BORDER}`, borderRadius: 8, padding: 12 }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 10 }}>
          <span style={{ fontSize: 12, fontWeight: 700, color: '#fff' }}>å¤šç©ºå¼·åº¦èµ°å‹¢</span>
        ...
        <ResponsiveContainer width="100%" height={120}>
          <AreaChart data={chartData} margin={{ top: 0, right: 0, left: -20, bottom: 0 }}>
            ...
            />
            <Area type="monotone" dataKey="bullScore" stroke={UP_COLOR} fill={`${UP_COLOR}22`} strokeWidth={1.5} name="å¤šæ–¹" dot={false} />
            <Area type="monotone" dataKey="bearScore" stroke={DN_COLOR} fill={`${DN_COLOR}22`} strokeWidth={1.5} name="ç©ºæ–¹" dot={false} />
          </AreaChart>
        </ResponsiveContainer>
      </div>
    </div>
