...
function toScanResult(r: any, side: 'bull' | 'bear'): ScanResult {
  return {
    ...
    yesterdayTrend: r.signalLabel ?? r.breakoutType ?? '中性',
...
export function AfterHoursScreener() {
  ...
  // Keep raw API results alongside ScanResult so we can pass score to card
  const bullRaw: any[]    = (bullData?.results ?? []).sort((a: any, b: any) => b.confidence - a.confidence);
  const bearRaw: any[]    = (bearData?.results ?? []).sort((a: any, b: any) => b.confidence - a.confidence);
  const bullResults: ScanResult[] = bullRaw.map((r: any) => toScanResult(r, 'bull'));
  const bearResults: ScanResult[] = bearRaw.map((r: any) => toScanResult(r, 'bear'));

  const filteredBull = bullFilter
    ? bullRaw.filter((r: any) => (r.signalLabel ?? r.breakoutType) === bullFilter)
    : bullRaw;
  const filteredBear = bearFilter
    ? bearRaw.filter((r: any) => (r.signalLabel ?? r.breakoutType) === bearFilter)
    : bearRaw;
          ...
          <>
            ...
            <div style={{ maxHeight: 560, overflowY: 'auto' }}>
              {filteredBull.length === 0 ? (
                ...
                filteredBull.map((r: any) => (
                  <div key={r.symbol} style={{ position: 'relative' }}>
                    <StockSignalCard
                      result={toScanResult(r, 'bull')}
                      mode="afterhours"
                      side="bull"
                      score={r.confidence ?? null}
                    />
                    {isLive && r.isLivePrice && (
          ...
          <>
            ...
            <div style={{ maxHeight: 560, overflowY: 'auto' }}>
              {filteredBear.length === 0 ? (
                ...
                filteredBear.map((r: any) => (
                  <div key={r.symbol} style={{ position: 'relative' }}>
                    <StockSignalCard
                      result={toScanResult(r, 'bear')}
                      mode="afterhours"
                      side="bear"
                      score={r.confidence ?? null}
                    />
