...
export default function AdSlot({ size, slotId }: AdSlotProps) {
  ...
  if (isDev) {
    return (
      <div style={{ width, height, border: '2px dashed var(--border)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'var(--text-secondary)', fontSize: 12, backgroundColor: 'var(--bg-secondary)', borderRadius: 4, userSelect: 'none' }}>
        廣告 {width}×{height}
      </div>
    );
  }
